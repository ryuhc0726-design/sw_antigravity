// Global Error Piping to Main Process
window.onerror = function (message, source, lineno, colno, error) {
  const errStr = `${message} at ${source}:${lineno}:${colno}`;
  window.api.logError(errStr);
};
window.onunhandledrejection = function (event) {
  window.api.logError('Unhandled rejection: ' + event.reason);
};
...
async function loadCsvPath() {
  try {
    currentCsvPath = await window.api.getCsvPath();
    elements.csvPath.textContent = getFileName(currentCsvPath);
    elements.csvPath.title = currentCsvPath;
  } catch (error) {
    console.error('Error getting CSV path:', error);
    elements.csvPath.textContent = 'CSV 경로 오류';
  }
}
...
async function loadData() {
  try {
    const result = await window.api.readCsv();
    if (result.success) {
      readingLogs = parseCSV(result.data);
      refreshUI();
    } else {
      alert('데이터를 읽어오는 데 실패했습니다: ' + result.error);
    }
  } catch (error) {
...
async function saveData() {
  try {
    const csvContent = stringifyCSV(readingLogs);
    const result = await window.api.writeCsv(csvContent);
    if (result.success) {
      return true;
    } else {
...
function setupEventListeners() {
  ...
  // CSV Path copy to clipboard on click
  elements.csvPath.addEventListener('click', () => {
    navigator.clipboard.writeText(currentCsvPath);
    alert('CSV 파일 절대 경로가 클립보드에 복사되었습니다:\n' + currentCsvPath);
  });
